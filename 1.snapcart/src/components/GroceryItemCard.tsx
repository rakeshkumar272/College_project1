...
function GroceryItemCard({ item }: { item: IGrocery }) {
  ...
  const cartItem = cartData.find(i => (i._id && i._id.toString() === itemId.toString()) || ((i as any).id && (i as any).id.toString() === itemId.toString()))
  return (
    <motion.div
    ...
    >
      ...
      <div className='p-4 flex flex-col flex-1'>
        ...
        {!cartItem ? <motion.button className='mt-4 flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white rounded-full py-2 text-sm font-medium transition-all'
          whileTap={{ scale: 0.96 }}
          onClick={() => dispatch(addToCart({ ...item, _id: itemId, quantity: 1, id: itemId } as any))}
        >
          ...
          >
            <button className='w-7 h-7 flex items-center justify-center rounded-full bg-green-100 hover:bg-green-200 transition-all' onClick={() => dispatch(decreaseQuantity(itemId))}><Minus size={16} className='text-green-700' /></button>
            <span className='text-sm font-semibold text-gray-800'>{cartItem.quantity}</span>
            <button className='w-7 h-7 flex items-center justify-center rounded-full bg-green-100 hover:bg-green-200 transition-all' onClick={() => dispatch(increaseQuantity(itemId))}><Plus size={16} className='text-green-700' /></button>
